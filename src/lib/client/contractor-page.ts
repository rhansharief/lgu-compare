...
function render(c: ContractorFile) {
  document.title = `${c.name} · Infra Money Tracker`;
  $('#c-name').textContent = c.name;
  const years = c.projects.map((p) => p[5]).filter((y): y is number => y != null);
  $('#c-sub').innerHTML = `${/^\d+$/.test(c.id) ? `PCAB licence no. ${esc(c.id)}` : 'No licence number listed'}${c.revoked ? ' · <span class="pill">Licence listed as revoked in DPWH data</span>' : ''}${years.length ? ` · projects from budget years ${Math.min(...years)}–${Math.max(...years)}` : ''}`;
  if (c.aliases) $('#c-aliases').textContent = `Also listed as: ${c.aliases.filter((a) => a !== c.name).join('; ')}`;
  ...
  $('#c-tiles').innerHTML = `
    <div class="card tile"><div class="label">Total awarded, all years</div><div class="value num">${fmtPesoCompact(c.total)}</div><div class="context">${c.jv ? `includes an equal share of ${c.jv} joint-venture contracts` : 'no joint ventures'}</div></div>
    ...
    <div class="card tile"><div class="label">Cities & municipalities</div><div class="value num">${c.lgus.length}</div><div class="context">${fmtPct(c.total ? c.mappedTotal / c.total : null)} of the amount could be placed in an LGU</div></div>`;
  ...
  $('#c-lgus').innerHTML = c.lgus.length
    ? `<div class="table-wrap"><table><thead><tr><th>City / municipality</th><th class="r">Projects</th><th class="r">Amount</th><th class="r">Share of LGU's DPWH spend</th></tr></thead><tbody>${
      ...
      }</tbody></table></div><p class="faint small" style="margin-top:8px">Share = this contractor's amount ÷ all DPWH project cost placed in that LGU, all years 2016–2026.</p>`
